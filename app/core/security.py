"""Encryption helpers for storing the HA long-lived access token at rest (ported from
event-creator's app/core/security.py - own copy per module, not a shared package, per the
credential storage ADR).

The stored Home Assistant token must never be persisted in plaintext. `CredentialCipher` wraps
Fernet (authenticated symmetric encryption from `cryptography`) to encrypt on write and decrypt on
read.

The cipher takes its key explicitly rather than reaching into global settings, so it's trivially
unit-testable with a throwaway generated key. Application code obtains the configured instance via
`get_credential_cipher()`, which reads `ENCRYPTION_KEY` (the platform's existing shared secret) from
settings.
"""
...
def get_credential_cipher() -> CredentialCipher:
    ...
    if not key:
        raise RuntimeError(
            "ENCRYPTION_KEY is not set - required to encrypt/decrypt the stored HA token. "
